...
0000000000400ee0 <phase_1>:
  ...
  # call strings_not_equal where %rdi = arg1 of phase_1, %rsi = 0x402400
  400ee9:	e8 4a 04 00 00       	call   401338 <strings_not_equal>
  # check if return value == 0 
  400eee:	85 c0                	test   %eax,%eax
  # if return value == 0, jump to 400ef7
  400ef0:	74 05                	je     400ef7 <phase_1+0x17>
  # otherwise call explode_bomb
...
0000000000400f43 <phase_3>:
  ...
  # must read more than 1 arg
  400f60:	83 f8 01             	cmp    $0x1,%eax
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	call   40143a <explode_bomb>
  # the first number must <= 7
...
0000000000400fce <func4>:
  ...
  # if rcx >= 0, rcx = 0x0, otherwise rcx = 0x1
...
000000000040100c <phase_4>:
  ...
  # if arg1 <= 14, jump, otherwise bomb!!!
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)
  401033:	76 05                	jbe    40103a <phase_4+0x2e>
  401035:	e8 00 04 00 00       	call   40143a <explode_bomb>
  # set arg 1, 2, 3, and call func4
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx
  40103f:	be 00 00 00 00       	mov    $0x0,%esi
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi
  401048:	e8 81 ff ff ff       	call   400fce <func4>
  # if return value != 0, bomb!!!
...
0000000000401062 <phase_5>:
  ...
  # if %edx in 0x30-39, the %edx will convert to real number
  ...
  # strings_not_equal, if return value == 1, bomb!!!
...
00000000004010f4 <phase_6>:
  ...
  # 6 nums store in top of stack
  # move num1 to r14
  40110b:	49 89 e6             	mov    %rsp,%r14
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d
  # rbp store rsp
  401114:	4c 89 ed             	mov    %r13,%rbp
  # store num[r13] in eax
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax
  40111b:	83 e8 01             	sub    $0x1,%eax
  # if eax > 5(num1 > 6), bomb!!!
  40111e:	83 f8 05             	cmp    $0x5,%eax
  401121:	76 05                	jbe    401128 <phase_6+0x34>
  401123:	e8 12 03 00 00       	call   40143a <explode_bomb>
  # i = r12, loop: for(i = 1; i != 6; i++) -> for (j = i + 1; j != 6; j++)
  401128:	41 83 c4 01          	add    $0x1,%r12d
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d
  401130:	74 21                	je     401153 <phase_6+0x5f>
  # eax = num[j] (num[1], num[2]... num[5]), ebx = i
  401132:	44 89 e3             	mov    %r12d,%ebx
  401135:	48 63 c3             	movslq %ebx,%rax
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax
  # if eax == num[0], bomb!!! so num[1 -> 5] != num[0]
  ...
  # loop3: store (0x6032d0 + 0x8 * (num[i] - 1)) in %rsp + 0x20 + 2 * (i - 1)
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2)
  40118d:	48 83 c6 04          	add    $0x4,%rsi
  401191:	48 83 fe 18          	cmp    $0x18,%rsi
  401195:	74 14                	je     4011ab <phase_6+0xb7>

  # if num[i] <= 1, jump to 401183
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx
  40119a:	83 f9 01             	cmp    $0x1,%ecx
  40119d:	7e e4                	jle    401183 <phase_6+0x8f>
  # otherwise, jump to 401176
...
000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  # mov arg1, arg2 to rbx(input) and rbp(answer) 
  40133c:	48 89 fb             	mov    %rdi,%rbx
  40133f:	48 89 f5             	mov    %rsi,%rbp
  # call string_length for rdi
  401342:	e8 d4 ff ff ff       	call   40131b <string_length>
  # mov length of arg1 to r12d(low 32 bits of r12)
  401347:	41 89 c4             	mov    %eax,%r12d
  # call string_length for rsi
  40134a:	48 89 ef             	mov    %rbp,%rdi
  40134d:	e8 c9 ff ff ff       	call   40131b <string_length>
  # %edx = 0x1
  401352:	ba 01 00 00 00       	mov    $0x1,%edx
  # check if length of arg1 == length of arg2
  401357:	41 39 c4             	cmp    %eax,%r12d
  # if not equal, jump to 0x40139b
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63>
  # eax = start address of string1
  40135c:	0f b6 03             	movzbl (%rbx),%eax
  # if al == 0 jump to 401388
  40135f:	84 c0                	test   %al,%al
  401361:	74 25                	je     401388 <strings_not_equal+0x50>
  # compare string1[0] and string2[0]
  401363:	3a 45 00             	cmp    0x0(%rbp),%al
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a>
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57>
  # compare string1[i] and string2[i]
  ...
  # if string not equal, %edx = 0x1 
  401396:	ba 01 00 00 00       	mov    $0x1,%edx
  # if length not equal, %eax = 0x1 
...
000000000040145c <read_six_numbers>:
  ...
  # rdx = arg2, in phase_2, arg2 is %rsp, eg. set rdx to top of stack
...
00000000004015c4 <phase_defused>:
  ...
  # don't jump when phase_6 defused
  ...
  # to defuse secret_phase, in phase_4 we must input an extra string "DrEvil"
